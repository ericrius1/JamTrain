...
- `src/game/multiplayer.ts` — sync helpers + listeners for `instrument`.
- `src/game/handSynth.ts` — instrument-driven voice creation; add Sparks voice.
- `src/game/rig.ts` — small helper `getPalmWorld(hand)`.
- `src/game/Game.ts` — drop PlasmaOrb wiring; install per-player visuals + CenterStage; route picker selections.

...
`r` is the one exception: it resets all tweak params (including audio mixer sliders) to their in-code defaults and works regardless of debug-overlay visibility, so a stuck/misconfigured session can always be recovered without first opening the overlay.
...
- Pressing `r` (any time, debug mode or not) resets every module's params back to the in-code defaults and clears persisted state.
